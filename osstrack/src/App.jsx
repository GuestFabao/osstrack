...
const calcFreq = (aluno, turmas) => {
  ...
  if (aluno.created_at && turmas) {
    ...
    if (turma && turma.dias) {
      ...
      const diasAtivos = turma.dias
        .split(/[\/,]/)
        .map((d) => d.trim().substring(0, 3));
      const indicesAtivos = diasAtivos
        .map((d) => diasMap[d])
...
function DbStatusBar({ status, turmasCount, erro }) {
  ...
  return (
    <div className="db-bar ok">
      <div className="db-dot" />
      Supabase conectado · {turmasCount} turmas carregadas
    </div>
  );
}
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (data.access_token)
        setMsg(
          "✓ Identidade confirmada. Troca de senha disponível na versão com Supabase SDK completo.",
        );
    } catch (e) {
...
function AlunoView({ aluno, turmas, carregarBanco }) {
  ...
  for (let i = 0; i < firstDayOfMonth; i++)
    days.push({ key: `e-${i}`, presente: false, temAula: false, label: "" });
  ...
  const isAniversario =
    ...
    (() => {
      ...
      const now = new Date();
      return (
        parseInt(mes) === now.getMonth() + 1 && parseInt(dia) === now.getDate()
      );
    })();
      ...
      {isAniversario && (
        ...
        >
          ...
          <div>
            ...
            >
              FELIZ ANIVERSÁRIO, {aluno.nome.split(" ")[0].toUpperCase()}!
            </div>
            <div style={{ fontSize: "0.8rem", color: "var(--muted)" }}>
              Toda a equipe Team Cruz BJJ deseja um ótimo dia! Oss! 🤜🤛
            </div>
          </div>
        </div>
      )}
      ...
      <div className="aluno-hero">
        ...
        <div>
          ...
          <div className="aluno-meta">
            Faixa{" "}
            <span style={{ color: FAIXA_COLORS[aluno.faixa] || "#ccc" }}>
              {aluno.faixa}
            </span>{" "}
            ({aluno.graus || 0} Graus) &nbsp;·&nbsp;
          ...
          {aluno.data_nascimento && (
            ...
            >
              🎂{" "}
              {new Date(aluno.data_nascimento + "T12:00:00").toLocaleDateString(
                "pt-BR",
        ...
        <div style={{ marginLeft: "auto", textAlign: "center" }}>
          <div
            style={{
              fontFamily: "'Bebas Neue', sans-serif",
              fontSize: "2.2rem",
              color: freq.pct >= 75 ? "var(--green)" : "var(--red)",
            }}
          >
            {freq.pct}%
          </div>
      ...
      <div className="grid-2">
          ...
          <div className="checkin-zone">
            ...
            >
              <span className="checkin-icon">
                {checkedIn
                  ? "✓"
                  : btnText === "ENCERRADO" || btnText === "DESCANSO"
                    ? "🔒"
                    : btnText === "AGUARDE"
                      ? "⏳"
                      : "👊"}
            ...
            <div className="aula-info">
              {turmaAluno ? (
                <>
                  Sua turma: <strong>{turmaAluno.nome}</strong> ·{" "}
                  {turmaAluno.horario}
                </>
              ) : (
                "Sem turma vinculada 🥋"
        ...
        <div className="card">
          <div className="section-title">
            Mês de {date.toLocaleDateString("pt-BR", { month: "long" })}
          </div>
          <div
            style={{
              display: "grid",
          ...
          <div className="hist-grid">
            {days.map((d) => (
              <div
                key={d.key}
                className={`hist-day ${d.presente ? "presente" : d.temAula ? "falta" : ""}`}
                title={d.key}
              >
                {d.label}
              </div>
          ...
          <div className="hist-legend">
            <span>
              <div className="dot" style={{ background: "#14532d" }} />
              Presente
            </span>
            <span>
              <div className="dot" style={{ background: "#1e1e1e" }} />
              Falta
            </span>
          </div>
          <div
                ...
                {[...presencas]
                  ...
                  .map((p) => (
                    <tr key={p.id}>
                      ...
                      <td>
                        {turmas.find((t) => t.id === p.turma_id)?.nome || "—"}
                      </td>
...
function AdminView({ turmas, alunos, carregarBanco, academiaAtual }) {
  ...
  const presencasHoje = alunos.filter((a) =>
    a.presencas?.some((p) => p.data === TODAY),
  );
  const faltasHoje = alunos.length - presencasHoje.length;
  const freqMedia = alunos.length
    ? Math.round(
        alunos.reduce((s, a) => s + calcFreq(a, turmas).pct, 0) / alunos.length,
      )
    : 0;
  const alunosFiltrados =
    filtroTurma === "Todas"
      ? alunos
      : alunos.filter(
          (a) => turmas.find((t) => t.id === a.turma_id)?.nome === filtroTurma,
        );
  const aniversariantes = getAniversariantesMes(alunos);

  const abrirModalAlunoNovo = () => {
    if (turmas.length === 0)
      return alert("Cadastre uma turma nesta unidade primeiro!");
    setFormAluno({
      id: null,
      nome: "",
      faixa: "Branca",
  ...
  const excluirAluno = async (id) => {
    if (!window.confirm("Excluir aluno? Todas as presenças serão apagadas."))
      return;
    try {
  ...
  const salvarAluno = async (e) => {
    e.preventDefault();
    if (!formAluno.nome || !formAluno.turma_id)
      return alert("Preencha nome e turma!");
    setSalvando(true);
    try {
      const payload = {
        nome: formAluno.nome,
  ...
  const abrirModalTurmaNova = () => {
    setFormTurma({ id: null, nome: "", horario: "", dias: "" });
    setModalTurmaOpen(true);
  };
  const abrirModalTurmaEditar = (t) => {
    setFormTurma({ id: t.id, nome: t.nome, horario: t.horario, dias: t.dias });
    setModalTurmaOpen(true);
  };

  const salvarTurma = async (e) => {
    e.preventDefault();
    if (!formTurma.nome || !formTurma.horario)
      return alert("Nome e horário obrigatórios!");
    setSalvando(true);
    try {
      if (formTurma.id) {
      ...
      {showExitModal && (
        <div className="modal-overlay">
          <div className="modal-box" style={{ textAlign: "center" }}>
            ...
            >
              Você realmente deseja fechar o OSS.TRACK?
            </p>
      ...
      {modalAlunoOpen && (
        <div className="modal-overlay">
          <div className="modal-box">
            <button
              className="btn-close"
              onClick={() => setModalAlunoOpen(false)}
            >
              ✕
            </button>
            <div className="modal-title">
              {formAluno.id ? "EDITAR ALUNO" : "NOVO ALUNO"}
            </div>
            <form onSubmit={salvarAluno}>
              ...
              >
                <div className="input-group">
                  <label>Faixa</label>
                  <select
                    value={formAluno.faixa}
                    onChange={(e) =>
                      setFormAluno({ ...formAluno, faixa: e.target.value })
                    }
                  >
                    {Object.keys(FAIXA_COLORS).map((f) => (
                ...
                <div className="input-group">
                  <label>Graus</label>
                  <select
                    value={formAluno.graus}
                    onChange={(e) =>
                      setFormAluno({ ...formAluno, graus: e.target.value })
                    }
                  >
                    {[0, 1, 2, 3, 4].map((g) => (
              ...
              <div className="input-group">
                <label>Turma</label>
                <select
                  value={formAluno.turma_id}
                  onChange={(e) =>
                    setFormAluno({ ...formAluno, turma_id: e.target.value })
                  }
                >
                  {turmas.map((t) => (
      ...
      {modalTurmaOpen && (
        <div className="modal-overlay">
          <div className="modal-box">
            ...
            <div className="modal-title">
              {formTurma.id ? "EDITAR TURMA" : "NOVA TURMA"}
            </div>
            <form onSubmit={salvarTurma}>
              ...
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormTurma({ ...formTurma, horario: e.target.value })
                  }
              ...
              <div className="input-group">
                ...
                <input
                  value={formTurma.dias}
                  onChange={(e) =>
                    setFormTurma({ ...formTurma, dias: e.target.value })
                  }
      ...
      {detalhe && (
        <>
          <div
            ...
            style={{ background: "rgba(0,0,0,0.5)", zIndex: 199 }}
          />
          <div className="detail-panel">
            <div>
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.8rem", color: "var(--muted)" }}>
                    Faixa{" "}
                    <span style={{ color: FAIXA_COLORS[detalhe.faixa] }}>
                      {detalhe.faixa}
                    </span>{" "}
                    · {detalhe.graus || 0} Graus
                  </div>
                  {detalhe.data_nascimento && (
                    ...
                    >
                      🎂{" "}
                      {new Date(
                        detalhe.data_nascimento + "T12:00:00",
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div className="scroll-list" style={{ maxHeight: "200px" }}>
                {detalhe.presencas?.length > 0 ? (
                  [...detalhe.presencas].reverse().map((p) => (
                    <div
                      key={p.id}
                      style={{
                        fontSize: "0.8rem",
            ...
            >
              <button
                ...
                onClick={() => abrirModalAlunoEditar(detalhe)}
              >
                ✏️ EDITAR DADOS
              </button>
              <button
                className="btn-danger"
                style={{ padding: "10px", fontSize: "1rem" }}
                onClick={() => excluirAluno(detalhe.id)}
              >
                🗑️ EXCLUIR ALUNO
              </button>
            </div>
        ...
        <>
          <div className="grid-4">
            ...
            <div className="card stat-card">
              ...
              <div className="stat-value green">{presencasHoje.length}</div>
              <div className="stat-sub">Registrados</div>
            </div>
            <div className="card stat-card">
              <div className="stat-label">Faltas Hoje</div>
              <div className="stat-value red">{faltasHoje}</div>
              <div className="stat-sub">Ausentes</div>
            </div>
            <div className="card stat-card">
              <div className="stat-label">Freq. Média</div>
              <div className="stat-value gold">{freqMedia}%</div>
              <div className="stat-sub">Geral da Academia</div>
            </div>
          </div>
          ...
          <div className="grid-2">
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <span
                              className="badge-faixa"
                              style={{ background: FAIXA_COLORS[a.faixa] }}
                            />
                            {a.nome}
                          </span>
                          <span
            ...
            <div className="card">
              ...
              <div className="scroll-list">
                {alunos.filter((a) => calcFreq(a, turmas).pct < 70).length ===
                0 ? (
                  <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>
                  ...
                  alunos
                    .filter((a) => calcFreq(a, turmas).pct < 70)
                    .map((a) => {
                      const f = calcFreq(a, turmas);
                      return (
                        <div
                      ...
                      >
                        <div
                          style={{
                            ...
                            alignItems: "center",
                            justifyContent: "center",
                            fontSize: a.isHoje ? "1.2rem" : "0.75rem",
                            fontWeight: 600,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            Dia {a.diaAniversario} ·{" "}
                            {turmas.find((t) => t.id === a.turma_id)?.nome} ·
                            Faixa{" "}
                            <span style={{ color: FAIXA_COLORS[a.faixa] }}>
                              {a.faixa}
                            </span>
                          </div>
                        </div>
                        <button
                          onClick={() =>
                            enviarParabensWhatsApp(
                              a,
                              academiaAtual?.nome || "Team Cruz BJJ",
                            )
                          }
                          style={{
                            background: "#16a34a",
        ...
        <div className="grid-2">
          <div className="card">
            <div className="section-title" style={{ color: "var(--green)" }}>
              ✓ Presentes ({presencasHoje.length})
            </div>
            <div className="scroll-list">
          ...
          <div className="card">
            ...
            <div className="scroll-list">
              {faltasHoje === 0 && alunos.length > 0 && (
                <p style={{ color: "var(--muted)", fontSize: "0.85rem" }}>
                  Todos presentes! 🎉
                </p>
              )}
              {alunos
                .filter((a) => !a.presencas?.some((p) => p.data === TODAY))
                .map((a) => (
                  <div
                    key={a.id}
                    className="hoje-item"
          ...
          >
            <div className="filters">
              {["Todas", ...turmas.map((t) => t.nome)].map((t) => (
                <button
                  key={t}
                  className={`filter-btn ${filtroTurma === t ? "active" : ""}`}
            ...
            <button
              className="btn-primary"
              style={{ width: "auto", padding: "8px 18px", fontSize: "0.9rem" }}
              onClick={abrirModalAlunoNovo}
            >
              + NOVO ALUNO
            </button>
          </div>
                  ...
                  {alunosFiltrados.map((a) => (
                    <tr key={a.id}>
                      <td>
                        ...
                        >
                          <div className="avatar">{a.foto}</div>
                          {a.nome}
                        </div>
                      </td>
                      <td>
...
export default function App() {
  ...
  const [academiaAtual, setAcademiaAtual] = useState(
    () => store.get("osstrack_admin_academia") || null,
  );
  ...
  const carregarBanco = async (idForcado = null) => {
    try {
      ...
      const todosOsAlunosData = await db.get(
        "alunos",
        "?select=*,presencas(*)&order=nome",
      );
      let listaGlobal = [];
      ...
      if (!idAtivo) {
        if (session?.role === "aluno") {
          ...
          idAtivo =
            memoriaAdmin?.id ||
            (Array.isArray(academiasData) && academiasData.length > 0
              ? academiasData[0].id
              : null);
        }
      }
      ...
      const turmasData = await db.get(
        "turmas",
        `?academia_id=eq.${idAtivo}&order=created_at`,
      );
      if (!Array.isArray(turmasData))
        throw new Error(turmasData?.message || "Erro nas turmas");
  ...
  const alunoLogado =
    session?.role === "aluno"
      ? todosAlunos.find((a) => a.id === session.alunoId)
      : null;
          ...
          <nav className="topnav">
            ...
            <div className="nav-right">
              ...
              <span className="nav-user">
                {session.username} ·{" "}
                {session.role === "admin" ? "Admin" : "Aluno"}
              </span>
              <button className="btn-logout" onClick={() => setSession(null)}>
                Sair
              </button>
            </div>
          </nav>
